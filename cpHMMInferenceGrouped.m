function cpHMMInferenceGrouped(projectName,InputDataPath,OutputDataPath,modelSpecs,varargin)
...
% set write path
outPrefix = [OutputDataPath filesep projectName filesep];
outDir = [outPrefix outSuffix];
mkdir(outDir);

%% %%%%%%%%%%%%%%%%%%%%%% Process/filter trace data %%%%%%%%%%%%%%%%%%%%%%%
[trace_struct_filtered, indexInfo] = filterTraces(inferenceOptions,analysis_traces);
...
for t = 1:length(indexInfo.indexVecUnique)
    ...
    for b = 1:inferenceOptions.nBoots
        ...
        if set_size < inferenceOptions.minDPperInf                    
            ...
            output.proteinBin = indexInfo.proteim_group_vec(t);
