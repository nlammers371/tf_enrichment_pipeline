...
for i = 1:size(set_frame_array,1)    
    ...
    for j = 1:numel(nc_x_vec)    
        ...
        if edge_qc_flag_vec(j) > 0  
            ...
            % draw snips    
            edge_null_protein_snips(:,:,j) = sample_snip(xc,yc,pt_snippet_size,protein_frame,nn_nc_mask);
            edge_null_mcp_snips(:,:,j) = sample_snip(xc,yc,pt_snippet_size,mcp_frame,nn_nc_mask);                                        
        end                  
...
%             edge_dev_vec = spot_edge_dist-nc_edge_dist_vec;
            % calculate weights
            wt_vec = exp(-.5*((drControl/(driftTol/PixelSize)).^2));%+((edge_dev_vec)/roi_spot).^2));
