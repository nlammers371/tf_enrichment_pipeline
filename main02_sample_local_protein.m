...
% 
    ...
    tic
    for i = 1:30%size(SetFrameArray,1)  
        ...
        %% %%%%%%%%% This part generates arrays that are parfor compatible%
        % Alas...the parallelized version is miles slower than the serial
        % one
        
        % Indexing vectors
        SliceIndices = RefStruct.particle_index_ref(samplingInfo.frame_set_indices);
        SliceSubIndices = RefStruct.particle_subindex_ref(samplingInfo.frame_set_indices);
        
        % spot vectors
        SpotXVec =  RefStruct.spot_x_ref(samplingInfo.frame_set_indices);
        SpotYVec =  RefStruct.spot_y_ref(samplingInfo.frame_set_indices);
        
        % serial control vectors        
        serial_x_temp = {spot_struct_protein(SliceIndices).serial_null_x_vec};
        serial_y_temp = {spot_struct_protein(SliceIndices).serial_null_y_vec};
        serial_frames_temp = {spot_struct_protein(SliceIndices).frames};        
        
        % generate slice-able version of Sampling results
        SamplingTemp = struct;
        SamplingFields = fieldnames(SamplingResults);
        for e = 1:length(SpotXVec)
            for f = 1:length(SamplingFields)
              SamplingTemp(e).(SamplingFields{f}) = NaN;
            end
        end    
        
%         j_pass = 1; % counter to track absolute position in iteration
        parfor j_pass = 1:length(SpotXVec)%samplingInfo.frame_set_indices        
            % get indexing info         
%             samplingInfo.spotIndex = RefStruct.particle_index_ref(j);
%             samplingInfo.spotSubIndex = RefStruct.particle_subindex_ref(j);

            x_spot = SpotXVec(j_pass);
            x_index = min([samplingInfo.xDim max([1 round(x_spot)])]);
            y_spot = SpotYVec(j_pass);
            ...
            if ~nucleus_mask_id
                ...
                SamplingTemp(j_pass).edge_qc_flag_vec = -1;            
                SamplingTemp(j_pass).serial_qc_flag_vec = -1;
            ...
            [SamplingTemp(j_pass).edge_null_nc_vec, ...
              SamplingTemp(j_pass).edge_null_x_vec,...
              SamplingTemp(j_pass).edge_null_y_vec, ...
              SamplingTemp(j_pass).edge_qc_flag_vec] = ...
              ...
            ...
            [SamplingTemp(j_pass).serial_null_edge_dist_vec,...
             SamplingTemp(j_pass).serial_null_x_vec,...
             SamplingTemp(j_pass).serial_null_y_vec,...
             SamplingTemp(j_pass).serial_qc_flag_vec]...
             ...
              = drawSerializedControlSpot(...
                  samplingInfo,nucleus_mask,samplingInfo.Frame,...
                  serial_frames_temp{j_pass}, ...
                  serial_x_temp{j_pass}, ...
                  serial_y_temp{j_pass});                             
             
             % increment
%              j_pass = j_pass + 1;
        end      
        % pass temp values to permanent sampling structure
        for e = 1:length(SpotXVec)
            for f = 1:length(SamplingFields)
              VecTemp = SamplingResults(i).(SamplingFields{f});
              VecTemp(e) = SamplingTemp(e).(SamplingFields{f});
              SamplingResults(i).(SamplingFields{f}) = VecTemp;
            end
        end  
        % for simplicity, pass stuff back to spot structure as well
        for e = 1:length(SpotXVec)          
            spot_struct_protein(SliceIndices(e)).serial_null_edge_dist_vec(SliceSubIndices(e))= SamplingResults(i).serial_null_edge_dist_vec(e); 
            spot_struct_protein(SliceIndices(e)).serial_null_x_vec(SliceSubIndices(e)) = SamplingResults(i).serial_null_x_vec(e);
            spot_struct_protein(SliceIndices(e)).serial_null_y_vec(SliceSubIndices(e)) = SamplingResults(i).serial_null_y_vec(e);
            spot_struct_protein(SliceIndices(e)).serial_qc_flag_vec(SliceSubIndices(e)) = SamplingResults(i).serial_qc_flag_vec(e);
        end
    end
    toc
