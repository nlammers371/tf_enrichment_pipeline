...
%     processed_nucleus_data = load(cn_filenames{i}); % processed particles  
    ...
    for e = 1:length(schnitzcells)
        ...
        if length(nc_frames) >= 1 % skip nuclei not desired nc range   
            ...
            s_cells(e_pass).zPosParticleDetrended = NaN(1,sum(nc_filter));
...
if threeD_flag
    ...
    % rescale 3D fluo
    mean2D = nanmean([nucleus_struct.fluo]);
    mean3D = nanmean([nucleus_struct.fluo3D]);
    for n = 1:length(nucleus_struct)
      nucleus_struct(n).fluo3D = nucleus_struct(n).fluo3D * mean2D / mean3D;
    end
else
    interp_fields = {'fluo'};
end
...
for i = 1:numel(nucleus_struct)
    ...
    if nucleus_struct(i).qc_flag == 1
        ...
        for  j = 1:numel(interp_fields)
            ...
            %Look for clusters of 5 or more NaNs
            kernel = [1,1,1,1,1];
            vec_nans = isnan(vec);
...
%             % find and remove suspsiciously large rises and falls
%             fluo_d = [0 diff(vec,1)];
%             if any(abs(fluo_d)>0.75*big_blip_thresh)
%               error('check')
%             end
%             vec(abs(fluo_d)>big_blip_thresh) = NaN;
            
            % interpolate to fill remaining NaN frames
...
warning('NL: removing all traces taht did not meet strict QC require,ent')
qc_vec = [nucleus_struct.qc_flag];
nucleus_struct = nucleus_struct(qc_vec==1);
