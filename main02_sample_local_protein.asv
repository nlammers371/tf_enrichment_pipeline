...
%         disp('segmenting nuclei...')   
    ...
    parfor i = 1:NIter%size(SetFrameArray,1)
        ...
        if size(samplingInfo.protein_stack,3) == samplingInfo.zDim+2
          samplingInfo.protein_stack = samplingInfo.protein_stack(:,:,2:end-1); 
        else
        ...
        if size(samplingInfo.mcp_stack,3) == samplingInfo.zDim+2
          samplingInfo.mcp_stack = samplingInfo.mcp_stack(:,:,2:end-1);
        else
          error('Unrecognized form of z-padding')
        end
        ...
        for j = samplingInfo.frame_set_indices 
            ...
            samplingSubInfo.z_spot_full = RefStruct.spot_z_ref(j)-1.0; % adjust for z padding
