...
PrefixCell = {'2021-05-12-zeiss980_airyscan_fast_mCherry_only'};
Tres = 4.5;
...
for p = 1:length(PrefixCell)
    ...
    master_struct(p).CompiledParticles = getCompiledParticles(liveExperiment);
    % save info
...
plot(tempArray(:,4),tempArray(:,5),'-s','Color','k');
...
p2 = plot(tempArray(:,10),tempArray(:,11),'-<');
