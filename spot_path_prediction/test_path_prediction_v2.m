...
n_vec = [spot_struct.N];
ParticlesFull = spot_struct(n_vec>=minDP);
...
for p = 1:length(ParticlesFull)
    % Frame-related stuff
    FramesRaw = ParticlesFull(p).frames;   
    
    % position stuff
    xFitVec = ParticlesFull(p).xPosParticle;
    yFitVec = ParticlesFull(p).yPosParticle;
    nan_ft = ~isnan(xFitVec);
    
    % remove NaNs
    FramesRaw = FramesRaw(nan_ft);
    xFitVec = xFitVec(nan_ft);
    yFitVec = yFitVec(nan_ft);
...
for p = 1:length(ParticlesFullLong)
    ...
    pIDVec = double(repelem(p,length(yFitVec))');
