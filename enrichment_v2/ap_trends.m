...
x_trend = mean_delta_array(t_index,:)/100;
xlb = -ste_delta_array(t_index,:)/100;
xub = ste_delta_array(t_index,:)/100;

y_trend = mean_fluo_array(t_index,:)/1e3;
ylb = -ste_fluo_array(t_index,:)/1e3;
yub = ste_fluo_array(t_index,:)/1e3;

errorbar(x_trend, y_trend, ylb, yub, xlb, xub,'o','CapSize',0,'Color','k','LineWidth',1);
scatter(x_trend,y_trend,50,'MarkerFaceColor','k','MarkerEdgeColor','k')

xlabel('Bcd-GFP enrichment at {\it hbP2P} locus')
ylabel('{\it hbP2P} fluorescence')

% formating
set(gca,'Fontsize',16);
set(gca,'Color',[228,221,209]/255) 
% grid on
corr_scatter.InvertHardcopy = 'off';
set(gcf,'color','w');

xlim([-.10 .40])
ylim([140 260])
saveas(corr_scatter,[FigurePath 'hbP2P_corr_scatter.png'])
