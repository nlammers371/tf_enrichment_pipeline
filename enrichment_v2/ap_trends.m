...
delta_rel_protein_array = NaN(nTimeBins,nAPBins,nBoots);
ctrl_protein_array = NaN(nTimeBins,nAPBins,nBoots);
delta_protein_array = NaN(nTimeBins,nAPBins,nBoots);
fluo_array = NaN(nTimeBins,nAPBins,nBoots);

for t = 1:length(time_bin_vec)-1
    for a = 1:length(ap_bin_vec)-1
        % generate AP filter
        ap_time_filter = ap_pos_vec_ft>=ap_bin_vec(a) & ap_pos_vec_ft<ap_bin_vec(a+1) & time_vec_ft>=time_bin_vec(t) & time_vec_ft<time_bin_vec(t+1);
        ...
        if sum(ap_time_filter) > 25
            ...
            for n = 1:nBoots
                ...
                nucleus_protein_array(t,a,n) = nanmean(nucleus_protein_vec_ft(boot_indices));   
...
%                                               ctrl_protein_vec_ft(boot_indices));
                ...
                ctrl_protein_array(t,a,n) = nanmean(ctrl_protein_vec_ft(boot_indices));
                delta_protein_array(t,a,n) = nanmean(spot_protein_vec_ft(boot_indices)-ctrl_protein_vec_ft(boot_indices));
                delta_rel_protein_array(t,a,n) = delta_protein_array(t,a,n)./ctrl_protein_array(t,a,n);
                fluo_array(t,a,n) = nanmean(fluo_vec_ft(boot_indices));
            end
        end
    end
end
...
%%% Spot intensity
%%%%%%%%%%%%%%%%%%%%%
mean_fluo_array = nanmean(fluo_array,3);
ste_fluo_array = nanstd(fluo_array,[],3);
...
for t = 1:length(time_plot)
    ...
    scatter(ap_plot,mean_fluo_array(t,:),'MarkerFaceColor',cm1(t,:),'MArkerEdgeColor','k')
end
% formating
set(gca,'Fontsize',14);
...
%%% Bcd enrichment
%%%%%%%%%%%%%%%%%%%%%
mean_delta_array = nanmean(delta_protein_array,3);
ste_delta_array = nanstd(delta_protein_array,[],3);
...
for t = 1:length(time_plot)
    ...
    scatter(ap_plot, mean_delta_array(t,:),'MarkerFaceColor',cm1(t,:),'MarkerEdgeColor','k')
end
...
%%% "nuclear" bcd
%%%%%%%%%%%%%%%%%%%%%
mean_nucleus_array = nanmean(ctrl_protein_array,3);
ste_nucleus_array = nanstd(ctrl_protein_array,[],3);
...
for t = 1:length(time_plot)
    ...
    scatter(ap_plot, mean_nucleus_array(t,:),'MarkerFaceColor',cm1(t,:),'MarkerEdgeColor','k')
end
...
%%% Bcd at spot
%%%%%%%%%%%%%%%%%%%%%
mean_rel_array = nanmean(delta_rel_protein_array,3);
ste_rel_array = nanstd(delta_rel_protein_array,[],3);
...
for t = 1:length(time_plot)
    ...
    scatter(ap_plot, mean_rel_array(t,:),'MarkerFaceColor',cm1(t,:),'MarkerEdgeColor','k')
end
...
scatter(ap_plot, mean_delta_array(t_index,:),'MarkerFaceColor',cm2(2,:),'MarkerEdgeColor','k')
