...
if strcmp(project,'Kr_GFP_eveBAC_mCherry')
    ft = ctrl_vec==1&ap_vec>.42&ap_vec<.55;
else
    ft = ctrl_vec==1;
end
...
p_vec = [];
for d = 1:numel(dist_cutoffs)
    % filter for data points with appropriate control
    dist = dist_cutoffs(d);
    pt_null_vec_plot = pt_null_vec_ctrl(dist_vec_ctrl>dist);
    pt_spot_vec_plot = pt_spot_vec_ctrl(dist_vec_ctrl>dist);
    
    [h,p] = kstest2(pt_spot_vec_plot,pt_null_vec_plot);
    p_vec = [p_vec p];
    lb = prctile([pt_null_vec_plot pt_spot_vec_plot],1);
    ub = prctile([pt_null_vec_plot pt_spot_vec_plot],99);
    bins = linspace(lb,ub,100);
    spot_ct = histc(pt_spot_vec_plot,bins);
    null_ct = histc(pt_null_vec_plot,bins);
    ...
    title(strvcat(['Distribution of ' protein_name '-' protein_fluor ' Concentrations'],...
           [ '    min dist: ' num2str(dist) ' \mu m, p=' num2str(p)]))
    grid on
    saveas(hist_fig,[FigPath 'hist_plots_' gene_name '_' protein_name '_dist_' num2str(dist) '.png']);    
end
...
if strcmp(project,'Kr_GFP_eveBAC_mCherry')
    snip_ctrl_vec = snip_struct_ctrl.ctrl_flags_final&dist_vec(nan_ft)>dist_lim&ap_vec(nan_ft)>.42&ap_vec(nan_ft)<.55;    
else
