...
io_ref_wt.exp_detection_counts = exp_counts;
io_ref_wt.predicted_detection_counts = pd_counts;
io_ref_wt.detection_fluo_axis = fluo_axis;
io_ref_wt.fluo_ref_curve = linspace(0,2*nanmax(fluo_vec_raw),1e3);
io_ref_wt.p_miss_ref_vec = fit(2)^fit(1) ./ (fit(2)^fit(1) + io_ref_wt.fluo_ref_curve.^fit(1));
