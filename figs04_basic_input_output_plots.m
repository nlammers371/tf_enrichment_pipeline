...
shuffle_vec_bio = sign(rand(size(burst_rise_bio_diff)) - .5);
...
[~, p_bio] = kstest2(burst_rise_bio_diff,burst_rise_bio_diff.*shuffle_vec_bio)
[~, p_spot] = kstest2(burst_rise_spot_diff,burst_rise_spot_diff.*shuffle_vec)
% resample biological control to be of equal size with others
burst_rise_bio_diff_rs = burst_rise_bio_diff(randsample(1:sum(burst_ft_control),numel(shuffle_vec),true),:);
% make figure
violin_fig = figure;
[~,L,MX,MED,bw]=violin([burst_rise_virt_diff burst_rise_swap_diff burst_rise_bio_diff_rs burst_rise_spot_diff],...
    'facecolor',[cmap1(3,:);cmap1(5,:);cmap1(end,:);cmap1(2,:)]);
ylim([-200 300])
set(gca,'xtick',1:4,'xticklabel',{'virtual spot','nearest neighbor','biological control','target locus'});
xtickangle(30)
