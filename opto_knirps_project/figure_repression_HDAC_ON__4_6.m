...
for i = 1:length(embryo)
    ...
    time_bin = double((0:max(frame_orig_long)))+0.5;
...
% perform simulation
%pd1 = makedist('Gamma','a',muhat(1),'b',muhat(2)); % bursting distribution
pd1 = makedist('Exponential',1.86); % bursting distribution
pd_delay = makedist('Normal','mu',2/3,'sigma',0.4);
...
% perform simulation
pd1 = makedist('Exponential',1); % bursting distribution
pd2 = makedist('Normal','mu',1,'sigma',0.2);
