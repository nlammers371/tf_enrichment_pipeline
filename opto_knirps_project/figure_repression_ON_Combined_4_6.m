...
projectNameCell = {'optokni_eve4+6_ON', 'optokni_eve4+6_ON_DELAY'}; 
%projectNameCell = {'optokni_eve4+6_ON'}; 
%projectNameCell = {'optokni_eve4+6_ON_DELAY'}; 
...
%% For "optokni_eve4+6_ON_DELAY"

if (strcmp(projectName,'optokni_eve4+6_ON_DELAY')) 
    % Embryo 2
    embryo(1).expID = 1;
    ...
    % Embryo 4
    %embryo(1).expID = 3;
    %embryo(1).frame_on = 38;
end

%% For 'optokni_eve4+6_ON'

if strcmp(projectName,'optokni_eve4+6_ON') 
    % Embryo 18
    %embryo(1).expID = 1;
...
%{
%% calculate integrated knirps concentration vs response time
...
%{
%% plot average silent time for completely silenced and unsilenced

mean(silence_time_full_frac(isnan(response_time_full_frac)))
mean(silence_time_full_frac(~isnan(response_time_full_frac)))

edges = linspace(0,10,5);

fig = figure;
histogram(silence_time_full_frac(isnan(response_time_full_frac)),edges,'Normalization','probability')
hold on
histogram(silence_time_full_frac(~isnan(response_time_full_frac)),edges,'Normalization','probability')
%}
...
%% plot two embryos together
%% integrated knirps concentration vs response time


x = knirps_int_last_on_all_cond;
y = response_time_all_cond;
filter = (y<=7)&~isnan(x)&(x<2.5E7);
...
memory_fig = figure;
%scatter(x(filter),y(filter),50,'filled','MarkerFaceColor',[234 194 100]/255,'MarkerEdgeColor',[0 .5 .5],'LineWidth',0.5)
scatter(x,y,50,'filled','MarkerFaceColor',[115 142 193]/255,'MarkerEdgeColor',[0 .5 .5],'LineWidth',0.5)
hold on
%plot(xRange,yResult,'-','LineWidth',2)
%plot(x,mdl)
xlabel('silenced knirps concentration (AU)');
...
binMin = 0.25E7;
binMax = 3E7;
edges = linspace(min(x),2.5E7,binNum);

[~,~,loc]=histcounts(xFit,edges);
meany = accumarray(loc(:),yFit(:))./accumarray(loc(:),1);
stdy = accumarray(loc(:),yFit(:),[],@std)./sqrt(accumarray(loc(:),1));
...
test_fig = figure;
%scatter(x(filter),y(filter),30,'filled','MarkerFaceColor','#BFBF99','MarkerEdgeColor',[0 .5 .5],'LineWidth',0.5)
scatter(xFit,yFit,25,'filled','MarkerFaceColor',[200 200 200]/256,'LineWidth',0.5)
hold on
errorbar(xmid, meany, stdy,'- .','CapSize',18,'MarkerSize',20,'Color','#D64D4D','LineWidth',1)

xlabel('integrated knirps before pert (AU)');
ylabel('response time (min)');
xlim([0 3E7])
ylim([0 7])
pbaspect([3 2 1])

%% integrated knirps vs mean fluo
...
x = knirps_int_last_on_all_cond;
y = fluo_mean_all_cond;
%y = response_time_all_cond;
cond = condition_filter;
filter = ~isnan(x)&(x<binMax)&(x>binMin);
...
%fig = figure;
%scatter(xFit(condFit == 1),yFit(condFit == 1),25,'filled','LineWidth',0.5)
%hold on
%scatter(xFit(condFit == 2),yFit(condFit == 2),25,'filled','LineWidth',0.5)


% plot the binned result

edges = linspace(binMin,binMax,binNum);
%edges = linspace(min(xFit),max(xFit),binNum);
...
%% mean knirps vs mean fluo

binNum = 11;
binMin = 7E5;
binMax = 10E5;

x = knirps_mean_all_cond;
y = fluo_mean_all_cond;
%y = response_time_all_cond;
cond = condition_filter;
filter = ~isnan(x)&(x<binMax)&(x>binMin);
...
y = response_time_all_cond;
cond = condition_filter;
filter = ~isnan(x)&(x<binMax)&(x>binMin)&(y<10);
%filter = ~isnan(x);
