...
%% plot mean vectors for nuclei on

time_groups = discretize(time_vec_long,time_bins); 
ap_filter_long = (ap_vec_long>=-0.02) & (ap_vec_long<=0.02);

fluo_vec_mean = NaN(1,length(time_bins)-1);
%fluo_vec_ste = NaN(1,length(time_bins)-1);
num_vec = NaN(1,length(time_bins)-1);

for i = 1:length(time_bins)-1
    fluo_vec_mean(i) = nanmean(fluo_zeros_long((time_groups == i)&(ap_filter_long)));
    num_vec(i) = sum(~isnan(fluo_zeros_long((time_groups == i)&(ap_filter_long))));
end
...
%% Figure: plot mean fluorescence vs time (only for the ones ON)
