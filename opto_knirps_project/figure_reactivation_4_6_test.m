...
% I don't like this embryo...? Concentration not high enough
% Embryo 9
embryo(1).expID = 1;
embryo(1).time_on = 20.48;
embryo(1).frame_on = 76;
expID = 1;
time_on = 20.48;
frame_on = 76;
...
fig = figure(3);
plot(time_vec(frame_on)-time_vec(last_on_long(data_filter)),time_vec(first_on_long(data_filter))-time_vec(frame_on),'o')
xlabel('last_on');
ylabel('first_on');
%xlim([0 10])
%ylim([0 6])