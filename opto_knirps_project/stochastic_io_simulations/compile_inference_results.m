...
%     if inf_ids(g) == 3 && knirps_mean_vec(g) > 8.8
%         error('asfa')
%     end

    zeros_vec(g) = unique(z_flag_vec(group_id_vec==group_index(g)&use_flags));
    
    kon_mean_vec(g) = nanmedian(kon_vec(group_id_vec==group_index(g)&use_flags));
    kon_ste_vec(g) = nanstd(kon_vec(group_id_vec==group_index(g)&use_flags));
    
    koff_mean_vec(g) = nanmedian(koff_vec(group_id_vec==group_index(g)&use_flags));
    ...
    r_mean_vec(g) = nanmedian(r_array(2,group_id_vec==group_index(g)&use_flags));
%     r_ste_vec(g) = nanstd(r_array(2,group_id_vec==group_index(g)&use_flags));
    r_ste_vec(g) = prctile(r_array(2,group_id_vec==group_index(g)&use_flags),75)-prctile(r_array(2,group_id_vec==group_index(g)&use_flags),25);
end    
