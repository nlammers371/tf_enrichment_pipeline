function sweepInfo = addGroundTruthFields(sweepInfo,io_ref_ra,io_ref_wt,io_ref_ON)
    ...
    % add opto data perturbed to keep stuff ON
    if ~isempty(io_ref_ON)        
        sweepInfo.fluo_time_mean_ON = io_ref_ON.fluo_time_mean;
        sweepInfo.fluo_time_ste_ON = io_ref_ON.fluo_time_ste;        
        
        sweepInfo.tf_profile_array_ON = io_ref_ON.knirps_array(start_i:end,:);               
        sweepInfo.ap_profile_vec_tf_ON = io_ref_ON.mean_ap;               
        sweepInfo.time_axis_ON = io_ref_ON.time_axis(start_i:end)';
    end
