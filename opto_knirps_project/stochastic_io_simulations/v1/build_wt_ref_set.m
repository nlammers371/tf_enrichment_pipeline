...
io_ref_wt.fluo_offset_array = NaN(length(time_index),length(spot_struct));
...
for i = 1:length(spot_struct)
    ...
    if time_vec(end)/60 >= time_bounds(2) && time_vec(1) <=time_bounds(1)/60 && first_time < 15 &&...
        ...
        % store
        io_ref_wt.fluo_offset_array(:,i) = offset_interp;
...
for n = 1:nBoots
    ...
    fluo_temp = io_ref_wt.fluo_array(:,boot_indices);
    knirps_temp = io_ref_wt.knirps_array(:,boot_indices);
    offset_temp = io_ref_wt.fluo_offset_array(:,boot_indices);
