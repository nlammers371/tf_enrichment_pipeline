...
for i = 1:length(spot_struct)
    ...
    if time_vec(end) - time_vec(1) >=3*60
...
ap_bins = linspace(0.55,0.675,11);
...
errorbar(ap_axis,fluo_vec_mean,fluo_vec_ste,'Color','k','CapSize',0);
%boundedline(ap_axis',fluo_vec_mean'*1e-5,fluo_vec_ste'*1e-5, '- .','nan', 'gap','alpha','cmap',brighten(gr,-0.5));
plot(ap_axis',fluo_vec_mean','-k')
scatter(ap_axis,fluo_vec_mean,50,'MarkerFaceColor',gr,'MarkerEdgeColor','k')
