...
for i = 1:numel(hmm_input_output)
    ...
    [~,~,nullBins] = histcounts(dt_sm,bulk_pt_prctile_vec);
    nan_filter = ~isnan(pt_nn_sm)&frame_vec>2&frame_vec<numel(frame_vec)-1;
    % find points of interest
    high_ids = find(nan_filter&nullBins>=numel(bulk_pt_prctile_vec)-2);
    low_ids = find(nan_filter&nullBins<=2);
    mid_ids = find(nan_filter&nullBins>=mid_bounds(1)&nullBins<=mid_bounds(2));
...
for i = 1:numel(name_cell)
    ...
    qc_fig = figure;
    hold on
    plot(nanmean(sampling_struct(i).ctrl_null_protein),'Color',blue)
    plot(nanmean(sampling_struct(i).trend_null_protein(qc_trend_indices,:)),'Color',red)
    ylabel([protein_name ' concentration (au)'])
%     yyaxis right
%     plot(nanmean(sampling_struct(i).ctrl_fluo),'--','Color',blue)
%     plot(nanmean(sampling_struct(i).trend_fluo(qc_trend_indices,:)),'--','Color',red)
%     ylabel([gene_name ' MS2 activity (au)'])
