...
for api = unique(ap_bin_vec)
    ...
    for p = 1:length(candidate_particles)
        ...
        time_vec = ft_array(:,1);
        fluo_vec = ft_array(:,2);
        % skip if we don't have continuous time info
        if max(diff(time_vec)>120)
            warning(['found gap in particle ' num2str(pID) '. Skipping.'])
            continue
        elseif min(diff(time_vec)<0.1)
            error('stitching issue')
        end
        ...
        single_trace_fits(iter).apPos = apVec(api);
        single_trace_fits(iter).time = time_vec;
        single_trace_fits(iter).fluo = fluo_vec;
        single_trace_fits(iter).promoter_probs3 = ss_mean;
        single_trace_fits(iter).promoter_state3 = ss_max;
        single_trace_fits(iter).promoter_probs2 = ss_mean2;
        single_trace_fits(iter).promoter_state2 = ss_max2;
...
% save
save([dataRoot subDir 'single_trace_fits.mat'])