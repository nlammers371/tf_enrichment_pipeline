...
error('need to add logic to allow for different inference groups')
...
for inf = 1:length(infResultList)
    ...
    for g = 1:length(groupID_index)
        ...
        parfor f = 1:length(fit_indices)
          ...
          viterbi_fits(f).time = spot_struct_protein(fit_indices(f)).timeInterp;
          viterbi_fits(f).protein = spot_struct_protein(fit_indices(f)).nuclear_protein_vecInterp;
          
          viterbi_fits(f).traceID = repelem(f,length(viterbi_fits(f).protein));
        end
    ...
    longArray = [double(trace_id_vec') double(time_vec'/60), double([viterbi_fits.fluo_exp]'), ...
        double([viterbi_fits.fluo_viterbi]'), double(([viterbi_fits.z_viterbi]'-1)>0) double(protein_vec')];
    longTable = array2table(longArray,'VariableNames',{'trace_id','time','spot_intensity','inferred_spot_intensity','inferred_promoter_state','protein_intensity'});

    writetable(longTable,[infResultList(inf).folder filesep infResultList(inf).name '_longform.csv'])
end
