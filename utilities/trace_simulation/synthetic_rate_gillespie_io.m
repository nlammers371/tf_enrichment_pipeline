...
function temp_gillespie = synthetic_rate_gillespie_io(seq_length,...
...
% calculate I/O curve
io_curve = tf_profile.^hill_coeff ./ (KD.^hill_coeff +  tf_profile.^hill_coeff);
...
for ts = 1:length(t_ref)
    ...
    while t_step < granularity
        ...
        if t_step < granularity 
            ...
            cs = randsample(options,1,true,RateMatrix(options(:),cs)');
