...
for i= 1:length(varargin)
    if strcmpi(varargin{i}, 'onlyApproved')
        onlyApproved = true; 
    elseif strcmpi(varargin{i}, 'inverseFlag')
        inverseFlag = true;
...
if onlyApproved %|| onlyUnapproved
    ...
    if inverseFlag
      readyLogicalArray = ~readyLogicalArray;
    end    
...
elseif customApproved
    ...
    approvalFlagRow = strcmpi(dataTypeTabContents(:,1),customApprovalFlag);
    ...
    approvalFlagLogicalArray(isnan(approvalFlagLogicalArray)) = 0;
    
    if inverseFlag
      approvalFlagLogicalArray = ~approvalFlagLogicalArray;
    end   
