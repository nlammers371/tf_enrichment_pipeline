function singleTraceFits = performSingleTraceFits(compiledResults, inferenceOptions, bootstrap_flag, ...
                              analysis_traces, trace_particle_index, nWorkersMax, resultsPath, infName, skipFitFlag)
...
%                      fieldnames(viterbi_out);
                    for fn = 1:numel(fnames)
                      if ~strcmpi(fnames{fn},'logL')
                        v_fits(f).(fnames{fn}) = NaN(1,length(fluo_values{f}));
                      else
                        v_fits(f).(fnames{fn}) = NaN;
                      end
                    end        
                end
            end
