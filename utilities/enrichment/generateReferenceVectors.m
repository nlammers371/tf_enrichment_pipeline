function refVecStruct = generateReferenceVectors(spot_struct_protein,refPath,has3DSpotInfo,ignoreQC)
...
if has3DSpotInfo
    refVecStruct.spot_x3D_ref = [spot_struct_protein.xPosParticle3D]; 
    refVecStruct.spot_y3D_ref = [spot_struct_protein.yPosParticle3D];
    refVecStruct.spot_z3D_ref = [spot_struct_protein.zPosParticle3D];
end

refVecStruct.setID_ref = [];
refVecStruct.master_nucleusID_ref = []; % list of nucleus indices
refVecStruct.particle_order_ref = []; % list of particle indices within nucleus_struct
...
for i = 1:numel(spot_struct_protein)
    ...
    refVecStruct.particle_order_ref = [refVecStruct.particle_order_ref repelem(i,numel(spot_struct_protein(i).frames))]; 
