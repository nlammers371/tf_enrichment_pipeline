...
%     qc_flag = hmm_input_output(i).mcp_qc_flag;
    ...
    if sum(ft) > window_size + 1 % && qc_flag
        iter = iter + 1;
...
for i = 1:numel(results_struct)  
    ...
    null_err_top = results_struct(i).swap_protein_mean + results_struct(i).swap_protein_ste;
    null_err_bottom = results_struct(i).swap_protein_mean - results_struct(i).swap_protein_ste;
    ...
    grid on
    saveas(input_output_fig,[figPath results_struct(i).fn '_in_out_swap.png'])
end
% save results
save([dataPath 'swap_input_output_results.mat'],'results_struct')